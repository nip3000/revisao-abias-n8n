...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateTransaction = async (id: string, transaction: Partial<Transaction>) => {
    try {
      console.log("AppContext: Calling updateTransactionService with:", { id, transaction });
      
      // Use the service which contains the credit card conversion logic
      const fullTransaction: Transaction = {
        ...
        creditCardId: transaction.creditCardId || transaction.credit_card_id,
        account_id: transaction.account_id,
        credit_card_id: transaction.credit_card_id
      };
      
      const result = await updateTransactionService(fullTransaction);
      
      if (result) {
        // Normal transaction update
        dispatch({ type: 'UPDATE_TRANSACTION', payload: result });
        console.log("AppContext: Transaction updated in state");
      } else {
        // Transaction was converted to credit card purchase, remove from transactions
        dispatch({ type: 'DELETE_TRANSACTION', payload: id });
        console.log("AppContext: Transaction converted to credit card purchase, removed from transactions");
      }
